window.RENEBOOK_WORLDCUP_2026_MATCHES = [
  {
    id: 1,
    date: "2026-06-11",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-11T19:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "MEX",
      flag: "MEX",
      es: "México",
      en: "Mexico"
    },
    away: {
      code: "RSA",
      flag: "RSA",
      es: "Sudáfrica",
      en: "South Africa"
    },
    stadium: {
      es: "Mexico City Stadium · Ciudad de México",
      en: "Mexico City Stadium · Mexico City"
    },
    es: {
      title: "México vs Sudáfrica",
      teams: "México vs Sudáfrica",
      location: "Mexico City Stadium · Ciudad de México",
      note: "Partido inaugural oficial del Mundial 2026."
    },
    en: {
      title: "Mexico vs South Africa",
      teams: "Mexico vs South Africa",
      location: "Mexico City Stadium · Mexico City",
      note: "Official opening match of the 2026 World Cup."
    }
  },
  {
    id: 2,
    date: "2026-06-11",
    officialTime: "22:00 ET",
    kickoffUtc: "2026-06-12T02:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "KOR",
      flag: "KOR",
      es: "Corea República",
      en: "Korea Republic"
    },
    away: {
      code: "CZE",
      flag: "CZE",
      es: "Chequia",
      en: "Czechia"
    },
    stadium: {
      es: "Guadalajara Stadium · Guadalajara",
      en: "Guadalajara Stadium · Guadalajara"
    },
    es: {
      title: "Corea República vs Chequia",
      teams: "Corea República vs Chequia",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Segundo partido del Grupo A."
    },
    en: {
      title: "Korea Republic vs Czechia",
      teams: "Korea Republic vs Czechia",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Second match of Group A."
    }
  },
  {
    id: 3,
    date: "2026-06-18",
    officialTime: "12:00 ET",
    kickoffUtc: "2026-06-18T16:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "CZE",
      flag: "CZE",
      es: "Chequia",
      en: "Czechia"
    },
    away: {
      code: "RSA",
      flag: "RSA",
      es: "Sudáfrica",
      en: "South Africa"
    },
    stadium: {
      es: "Atlanta Stadium · Atlanta",
      en: "Atlanta Stadium · Atlanta"
    },
    es: {
      title: "Chequia vs Sudáfrica",
      teams: "Chequia vs Sudáfrica",
      location: "Atlanta Stadium · Atlanta",
      note: "Partido del Grupo A."
    },
    en: {
      title: "Czechia vs South Africa",
      teams: "Czechia vs South Africa",
      location: "Atlanta Stadium · Atlanta",
      note: "Group A match."
    }
  },
  {
    id: 4,
    date: "2026-06-18",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-19T01:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "MEX",
      flag: "MEX",
      es: "México",
      en: "Mexico"
    },
    away: {
      code: "KOR",
      flag: "KOR",
      es: "Corea República",
      en: "Korea Republic"
    },
    stadium: {
      es: "Guadalajara Stadium · Guadalajara",
      en: "Guadalajara Stadium · Guadalajara"
    },
    es: {
      title: "México vs Corea República",
      teams: "México vs Corea República",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Segundo partido de México en el Grupo A."
    },
    en: {
      title: "Mexico vs Korea Republic",
      teams: "Mexico vs Korea Republic",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Mexico’s second match in Group A."
    }
  },
  {
    id: 5,
    date: "2026-06-24",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-25T01:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "CZE",
      flag: "CZE",
      es: "Chequia",
      en: "Czechia"
    },
    away: {
      code: "MEX",
      flag: "MEX",
      es: "México",
      en: "Mexico"
    },
    stadium: {
      es: "Mexico City Stadium · Ciudad de México",
      en: "Mexico City Stadium · Mexico City"
    },
    es: {
      title: "Chequia vs México",
      teams: "Chequia vs México",
      location: "Mexico City Stadium · Ciudad de México",
      note: "Cierre de fase de grupos para México."
    },
    en: {
      title: "Czechia vs Mexico",
      teams: "Czechia vs Mexico",
      location: "Mexico City Stadium · Mexico City",
      note: "Mexico’s final group stage match."
    }
  },
  {
    id: 6,
    date: "2026-06-24",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-25T01:00:00Z",
    stage: "Group Stage",
    group: "A",
    source: "FIFA",
    home: {
      code: "RSA",
      flag: "RSA",
      es: "Sudáfrica",
      en: "South Africa"
    },
    away: {
      code: "KOR",
      flag: "KOR",
      es: "Corea República",
      en: "Korea Republic"
    },
    stadium: {
      es: "Monterrey Stadium · Monterrey",
      en: "Monterrey Stadium · Monterrey"
    },
    es: {
      title: "Sudáfrica vs Corea República",
      teams: "Sudáfrica vs Corea República",
      location: "Monterrey Stadium · Monterrey",
      note: "Partido final del Grupo A."
    },
    en: {
      title: "South Africa vs Korea Republic",
      teams: "South Africa vs Korea Republic",
      location: "Monterrey Stadium · Monterrey",
      note: "Final Group A match."
    }
  },
  {
    id: 7,
    date: "2026-06-12",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-12T19:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "CAN",
      flag: "CAN",
      es: "Canadá",
      en: "Canada"
    },
    away: {
      code: "BIH",
      flag: "BIH",
      es: "Bosnia y Herzegovina",
      en: "Bosnia and Herzegovina"
    },
    stadium: {
      es: "Toronto Stadium · Toronto",
      en: "Toronto Stadium · Toronto"
    },
    es: {
      title: "Canadá vs Bosnia y Herzegovina",
      teams: "Canadá vs Bosnia y Herzegovina",
      location: "Toronto Stadium · Toronto",
      note: "Primer partido del Grupo B."
    },
    en: {
      title: "Canada vs Bosnia and Herzegovina",
      teams: "Canada vs Bosnia and Herzegovina",
      location: "Toronto Stadium · Toronto",
      note: "First match of Group B."
    }
  },
  {
    id: 8,
    date: "2026-06-13",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-13T19:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "QAT",
      flag: "QAT",
      es: "Qatar",
      en: "Qatar"
    },
    away: {
      code: "SUI",
      flag: "SUI",
      es: "Suiza",
      en: "Switzerland"
    },
    stadium: {
      es: "San Francisco Bay Stadium · Santa Clara",
      en: "San Francisco Bay Stadium · Santa Clara"
    },
    es: {
      title: "Qatar vs Suiza",
      teams: "Qatar vs Suiza",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Partido del Grupo B."
    },
    en: {
      title: "Qatar vs Switzerland",
      teams: "Qatar vs Switzerland",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Group B match."
    }
  },
  {
    id: 9,
    date: "2026-06-18",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-18T19:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "SUI",
      flag: "SUI",
      es: "Suiza",
      en: "Switzerland"
    },
    away: {
      code: "BIH",
      flag: "BIH",
      es: "Bosnia y Herzegovina",
      en: "Bosnia and Herzegovina"
    },
    stadium: {
      es: "Los Angeles Stadium · Los Angeles",
      en: "Los Angeles Stadium · Los Angeles"
    },
    es: {
      title: "Suiza vs Bosnia y Herzegovina",
      teams: "Suiza vs Bosnia y Herzegovina",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Partido del Grupo B."
    },
    en: {
      title: "Switzerland vs Bosnia and Herzegovina",
      teams: "Switzerland vs Bosnia and Herzegovina",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Group B match."
    }
  },
  {
    id: 10,
    date: "2026-06-18",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-18T22:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "CAN",
      flag: "CAN",
      es: "Canadá",
      en: "Canada"
    },
    away: {
      code: "QAT",
      flag: "QAT",
      es: "Qatar",
      en: "Qatar"
    },
    stadium: {
      es: "BC Place Vancouver · Vancouver",
      en: "BC Place Vancouver · Vancouver"
    },
    es: {
      title: "Canadá vs Qatar",
      teams: "Canadá vs Qatar",
      location: "BC Place Vancouver · Vancouver",
      note: "Segundo partido de Canadá en el Grupo B."
    },
    en: {
      title: "Canada vs Qatar",
      teams: "Canada vs Qatar",
      location: "BC Place Vancouver · Vancouver",
      note: "Canada’s second match in Group B."
    }
  },
  {
    id: 11,
    date: "2026-06-24",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-24T19:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "SUI",
      flag: "SUI",
      es: "Suiza",
      en: "Switzerland"
    },
    away: {
      code: "CAN",
      flag: "CAN",
      es: "Canadá",
      en: "Canada"
    },
    stadium: {
      es: "BC Place Vancouver · Vancouver",
      en: "BC Place Vancouver · Vancouver"
    },
    es: {
      title: "Suiza vs Canadá",
      teams: "Suiza vs Canadá",
      location: "BC Place Vancouver · Vancouver",
      note: "Partido final del Grupo B para Canadá."
    },
    en: {
      title: "Switzerland vs Canada",
      teams: "Switzerland vs Canada",
      location: "BC Place Vancouver · Vancouver",
      note: "Canada’s final Group B match."
    }
  },
  {
    id: 12,
    date: "2026-06-24",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-24T19:00:00Z",
    stage: "Group Stage",
    group: "B",
    source: "FIFA",
    home: {
      code: "BIH",
      flag: "BIH",
      es: "Bosnia y Herzegovina",
      en: "Bosnia and Herzegovina"
    },
    away: {
      code: "QAT",
      flag: "QAT",
      es: "Qatar",
      en: "Qatar"
    },
    stadium: {
      es: "Seattle Stadium · Seattle",
      en: "Seattle Stadium · Seattle"
    },
    es: {
      title: "Bosnia y Herzegovina vs Qatar",
      teams: "Bosnia y Herzegovina vs Qatar",
      location: "Seattle Stadium · Seattle",
      note: "Partido final del Grupo B."
    },
    en: {
      title: "Bosnia and Herzegovina vs Qatar",
      teams: "Bosnia and Herzegovina vs Qatar",
      location: "Seattle Stadium · Seattle",
      note: "Final Group B match."
    }
  },
  {
    id: 13,
    date: "2026-06-13",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-13T22:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "BRA",
      flag: "BRA",
      es: "Brasil",
      en: "Brazil"
    },
    away: {
      code: "MAR",
      flag: "MAR",
      es: "Marruecos",
      en: "Morocco"
    },
    stadium: {
      es: "New York New Jersey Stadium · East Rutherford",
      en: "New York New Jersey Stadium · East Rutherford"
    },
    es: {
      title: "Brasil vs Marruecos",
      teams: "Brasil vs Marruecos",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Partido inicial del Grupo C."
    },
    en: {
      title: "Brazil vs Morocco",
      teams: "Brazil vs Morocco",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Opening match of Group C."
    }
  },
  {
    id: 14,
    date: "2026-06-13",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-14T01:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "HAI",
      flag: "HAI",
      es: "Haití",
      en: "Haiti"
    },
    away: {
      code: "SCO",
      flag: "SCO",
      es: "Escocia",
      en: "Scotland"
    },
    stadium: {
      es: "Boston Stadium · Foxborough",
      en: "Boston Stadium · Foxborough"
    },
    es: {
      title: "Haití vs Escocia",
      teams: "Haití vs Escocia",
      location: "Boston Stadium · Foxborough",
      note: "Partido del Grupo C."
    },
    en: {
      title: "Haiti vs Scotland",
      teams: "Haiti vs Scotland",
      location: "Boston Stadium · Foxborough",
      note: "Group C match."
    }
  },
  {
    id: 15,
    date: "2026-06-19",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-19T22:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "SCO",
      flag: "SCO",
      es: "Escocia",
      en: "Scotland"
    },
    away: {
      code: "MAR",
      flag: "MAR",
      es: "Marruecos",
      en: "Morocco"
    },
    stadium: {
      es: "Boston Stadium · Foxborough",
      en: "Boston Stadium · Foxborough"
    },
    es: {
      title: "Escocia vs Marruecos",
      teams: "Escocia vs Marruecos",
      location: "Boston Stadium · Foxborough",
      note: "Partido del Grupo C."
    },
    en: {
      title: "Scotland vs Morocco",
      teams: "Scotland vs Morocco",
      location: "Boston Stadium · Foxborough",
      note: "Group C match."
    }
  },
  {
    id: 16,
    date: "2026-06-19",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-20T01:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "BRA",
      flag: "BRA",
      es: "Brasil",
      en: "Brazil"
    },
    away: {
      code: "HAI",
      flag: "HAI",
      es: "Haití",
      en: "Haiti"
    },
    stadium: {
      es: "Philadelphia Stadium · Philadelphia",
      en: "Philadelphia Stadium · Philadelphia"
    },
    es: {
      title: "Brasil vs Haití",
      teams: "Brasil vs Haití",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Segundo partido de Brasil en el Grupo C."
    },
    en: {
      title: "Brazil vs Haiti",
      teams: "Brazil vs Haiti",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Brazil’s second match in Group C."
    }
  },
  {
    id: 17,
    date: "2026-06-24",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-24T22:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "SCO",
      flag: "SCO",
      es: "Escocia",
      en: "Scotland"
    },
    away: {
      code: "BRA",
      flag: "BRA",
      es: "Brasil",
      en: "Brazil"
    },
    stadium: {
      es: "Miami Stadium · Miami",
      en: "Miami Stadium · Miami"
    },
    es: {
      title: "Escocia vs Brasil",
      teams: "Escocia vs Brasil",
      location: "Miami Stadium · Miami",
      note: "Partido final del Grupo C para Brasil."
    },
    en: {
      title: "Scotland vs Brazil",
      teams: "Scotland vs Brazil",
      location: "Miami Stadium · Miami",
      note: "Brazil’s final Group C match."
    }
  },
  {
    id: 18,
    date: "2026-06-24",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-24T22:00:00Z",
    stage: "Group Stage",
    group: "C",
    source: "FIFA",
    home: {
      code: "MAR",
      flag: "MAR",
      es: "Marruecos",
      en: "Morocco"
    },
    away: {
      code: "HAI",
      flag: "HAI",
      es: "Haití",
      en: "Haiti"
    },
    stadium: {
      es: "Atlanta Stadium · Atlanta",
      en: "Atlanta Stadium · Atlanta"
    },
    es: {
      title: "Marruecos vs Haití",
      teams: "Marruecos vs Haití",
      location: "Atlanta Stadium · Atlanta",
      note: "Partido final del Grupo C."
    },
    en: {
      title: "Morocco vs Haiti",
      teams: "Morocco vs Haiti",
      location: "Atlanta Stadium · Atlanta",
      note: "Final Group C match."
    }
  },
  {
    id: 19,
    date: "2026-06-12",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-13T01:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "USA",
      flag: "USA",
      es: "Estados Unidos",
      en: "United States"
    },
    away: {
      code: "PAR",
      flag: "PAR",
      es: "Paraguay",
      en: "Paraguay"
    },
    stadium: {
      es: "Los Angeles Stadium · Los Angeles",
      en: "Los Angeles Stadium · Los Angeles"
    },
    es: {
      title: "Estados Unidos vs Paraguay",
      teams: "Estados Unidos vs Paraguay",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Primer partido de Estados Unidos en el Grupo D."
    },
    en: {
      title: "United States vs Paraguay",
      teams: "United States vs Paraguay",
      location: "Los Angeles Stadium · Los Angeles",
      note: "United States’ opening match in Group D."
    }
  },
  {
    id: 20,
    date: "2026-06-13",
    officialTime: "00:00 ET",
    kickoffUtc: "2026-06-13T04:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "AUS",
      flag: "AUS",
      es: "Australia",
      en: "Australia"
    },
    away: {
      code: "TUR",
      flag: "TUR",
      es: "Türkiye",
      en: "Türkiye"
    },
    stadium: {
      es: "BC Place Vancouver · Vancouver",
      en: "BC Place Vancouver · Vancouver"
    },
    es: {
      title: "Australia vs Türkiye",
      teams: "Australia vs Türkiye",
      location: "BC Place Vancouver · Vancouver",
      note: "Partido del Grupo D."
    },
    en: {
      title: "Australia vs Türkiye",
      teams: "Australia vs Türkiye",
      location: "BC Place Vancouver · Vancouver",
      note: "Group D match."
    }
  },
  {
    id: 21,
    date: "2026-06-19",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-19T19:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "USA",
      flag: "USA",
      es: "Estados Unidos",
      en: "United States"
    },
    away: {
      code: "AUS",
      flag: "AUS",
      es: "Australia",
      en: "Australia"
    },
    stadium: {
      es: "Seattle Stadium · Seattle",
      en: "Seattle Stadium · Seattle"
    },
    es: {
      title: "Estados Unidos vs Australia",
      teams: "Estados Unidos vs Australia",
      location: "Seattle Stadium · Seattle",
      note: "Segundo partido de Estados Unidos en el Grupo D."
    },
    en: {
      title: "United States vs Australia",
      teams: "United States vs Australia",
      location: "Seattle Stadium · Seattle",
      note: "United States’ second match in Group D."
    }
  },
  {
    id: 22,
    date: "2026-06-20",
    officialTime: "00:00 ET",
    kickoffUtc: "2026-06-20T04:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "TUR",
      flag: "TUR",
      es: "Türkiye",
      en: "Türkiye"
    },
    away: {
      code: "PAR",
      flag: "PAR",
      es: "Paraguay",
      en: "Paraguay"
    },
    stadium: {
      es: "San Francisco Bay Stadium · Santa Clara",
      en: "San Francisco Bay Stadium · Santa Clara"
    },
    es: {
      title: "Türkiye vs Paraguay",
      teams: "Türkiye vs Paraguay",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Partido del Grupo D."
    },
    en: {
      title: "Türkiye vs Paraguay",
      teams: "Türkiye vs Paraguay",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Group D match."
    }
  },
  {
    id: 23,
    date: "2026-06-25",
    officialTime: "22:00 ET",
    kickoffUtc: "2026-06-26T02:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "USA",
      flag: "USA",
      es: "Estados Unidos",
      en: "United States"
    },
    away: {
      code: "TUR",
      flag: "TUR",
      es: "Türkiye",
      en: "Türkiye"
    },
    stadium: {
      es: "Los Angeles Stadium · Los Angeles",
      en: "Los Angeles Stadium · Los Angeles"
    },
    es: {
      title: "Estados Unidos vs Türkiye",
      teams: "Estados Unidos vs Türkiye",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Cierre de fase de grupos para Estados Unidos."
    },
    en: {
      title: "United States vs Türkiye",
      teams: "United States vs Türkiye",
      location: "Los Angeles Stadium · Los Angeles",
      note: "United States’ final group stage match."
    }
  },
  {
    id: 24,
    date: "2026-06-25",
    officialTime: "22:00 ET",
    kickoffUtc: "2026-06-26T02:00:00Z",
    stage: "Group Stage",
    group: "D",
    source: "FIFA",
    home: {
      code: "PAR",
      flag: "PAR",
      es: "Paraguay",
      en: "Paraguay"
    },
    away: {
      code: "AUS",
      flag: "AUS",
      es: "Australia",
      en: "Australia"
    },
    stadium: {
      es: "San Francisco Bay Stadium · Santa Clara",
      en: "San Francisco Bay Stadium · Santa Clara"
    },
    es: {
      title: "Paraguay vs Australia",
      teams: "Paraguay vs Australia",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Partido final del Grupo D."
    },
    en: {
      title: "Paraguay vs Australia",
      teams: "Paraguay vs Australia",
      location: "San Francisco Bay Stadium · Santa Clara",
      note: "Final Group D match."
    }
  },
  {
    id: 25,
    date: "2026-06-14",
    officialTime: "13:00 ET",
    kickoffUtc: "2026-06-14T17:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "GER",
      flag: "GER",
      es: "Alemania",
      en: "Germany"
    },
    away: {
      code: "CUW",
      flag: "CUW",
      es: "Curaçao",
      en: "Curaçao"
    },
    stadium: {
      es: "Houston Stadium · Houston",
      en: "Houston Stadium · Houston"
    },
    es: {
      title: "Alemania vs Curaçao",
      teams: "Alemania vs Curaçao",
      location: "Houston Stadium · Houston",
      note: "Primer partido de Alemania en el Grupo E."
    },
    en: {
      title: "Germany vs Curaçao",
      teams: "Germany vs Curaçao",
      location: "Houston Stadium · Houston",
      note: "Germany’s opening match in Group E."
    }
  },
  {
    id: 26,
    date: "2026-06-14",
    officialTime: "19:00 ET",
    kickoffUtc: "2026-06-14T23:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "CIV",
      flag: "CIV",
      es: "Costa de Marfil",
      en: "Côte d'Ivoire"
    },
    away: {
      code: "ECU",
      flag: "ECU",
      es: "Ecuador",
      en: "Ecuador"
    },
    stadium: {
      es: "Philadelphia Stadium · Philadelphia",
      en: "Philadelphia Stadium · Philadelphia"
    },
    es: {
      title: "Costa de Marfil vs Ecuador",
      teams: "Costa de Marfil vs Ecuador",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Partido del Grupo E."
    },
    en: {
      title: "Côte d'Ivoire vs Ecuador",
      teams: "Côte d'Ivoire vs Ecuador",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Group E match."
    }
  },
  {
    id: 27,
    date: "2026-06-20",
    officialTime: "16:00 ET",
    kickoffUtc: "2026-06-20T20:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "GER",
      flag: "GER",
      es: "Alemania",
      en: "Germany"
    },
    away: {
      code: "CIV",
      flag: "CIV",
      es: "Costa de Marfil",
      en: "Côte d'Ivoire"
    },
    stadium: {
      es: "Toronto Stadium · Toronto",
      en: "Toronto Stadium · Toronto"
    },
    es: {
      title: "Alemania vs Costa de Marfil",
      teams: "Alemania vs Costa de Marfil",
      location: "Toronto Stadium · Toronto",
      note: "Segundo partido de Alemania en el Grupo E."
    },
    en: {
      title: "Germany vs Côte d'Ivoire",
      teams: "Germany vs Côte d'Ivoire",
      location: "Toronto Stadium · Toronto",
      note: "Germany’s second match in Group E."
    }
  },
  {
    id: 28,
    date: "2026-06-20",
    officialTime: "20:00 ET",
    kickoffUtc: "2026-06-21T00:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "ECU",
      flag: "ECU",
      es: "Ecuador",
      en: "Ecuador"
    },
    away: {
      code: "CUW",
      flag: "CUW",
      es: "Curaçao",
      en: "Curaçao"
    },
    stadium: {
      es: "Kansas City Stadium · Kansas City",
      en: "Kansas City Stadium · Kansas City"
    },
    es: {
      title: "Ecuador vs Curaçao",
      teams: "Ecuador vs Curaçao",
      location: "Kansas City Stadium · Kansas City",
      note: "Partido del Grupo E."
    },
    en: {
      title: "Ecuador vs Curaçao",
      teams: "Ecuador vs Curaçao",
      location: "Kansas City Stadium · Kansas City",
      note: "Group E match."
    }
  },
  {
    id: 29,
    date: "2026-06-25",
    officialTime: "16:00 ET",
    kickoffUtc: "2026-06-25T20:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "ECU",
      flag: "ECU",
      es: "Ecuador",
      en: "Ecuador"
    },
    away: {
      code: "GER",
      flag: "GER",
      es: "Alemania",
      en: "Germany"
    },
    stadium: {
      es: "New York New Jersey Stadium · East Rutherford",
      en: "New York New Jersey Stadium · East Rutherford"
    },
    es: {
      title: "Ecuador vs Alemania",
      teams: "Ecuador vs Alemania",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Partido final del Grupo E para Alemania."
    },
    en: {
      title: "Ecuador vs Germany",
      teams: "Ecuador vs Germany",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Germany’s final Group E match."
    }
  },
  {
    id: 30,
    date: "2026-06-25",
    officialTime: "16:00 ET",
    kickoffUtc: "2026-06-25T20:00:00Z",
    stage: "Group Stage",
    group: "E",
    source: "FIFA",
    home: {
      code: "CUW",
      flag: "CUW",
      es: "Curaçao",
      en: "Curaçao"
    },
    away: {
      code: "CIV",
      flag: "CIV",
      es: "Costa de Marfil",
      en: "Côte d'Ivoire"
    },
    stadium: {
      es: "Philadelphia Stadium · Philadelphia",
      en: "Philadelphia Stadium · Philadelphia"
    },
    es: {
      title: "Curaçao vs Costa de Marfil",
      teams: "Curaçao vs Costa de Marfil",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Partido final del Grupo E."
    },
    en: {
      title: "Curaçao vs Côte d'Ivoire",
      teams: "Curaçao vs Côte d'Ivoire",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Final Group E match."
    }
  },
  {
    id: 31,
    date: "2026-06-14",
    officialTime: "16:00 ET",
    kickoffUtc: "2026-06-14T20:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "NED",
      flag: "NED",
      es: "Países Bajos",
      en: "Netherlands"
    },
    away: {
      code: "JPN",
      flag: "JPN",
      es: "Japón",
      en: "Japan"
    },
    stadium: {
      es: "Dallas Stadium · Dallas",
      en: "Dallas Stadium · Dallas"
    },
    es: {
      title: "Países Bajos vs Japón",
      teams: "Países Bajos vs Japón",
      location: "Dallas Stadium · Dallas",
      note: "Primer partido del Grupo F."
    },
    en: {
      title: "Netherlands vs Japan",
      teams: "Netherlands vs Japan",
      location: "Dallas Stadium · Dallas",
      note: "Opening match of Group F."
    }
  },
  {
    id: 32,
    date: "2026-06-14",
    officialTime: "22:00 ET",
    kickoffUtc: "2026-06-15T02:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "TUN",
      flag: "TUN",
      es: "Túnez",
      en: "Tunisia"
    },
    away: {
      code: "SWE",
      flag: "SWE",
      es: "Suecia",
      en: "Sweden"
    },
    stadium: {
      es: "Monterrey Stadium · Monterrey",
      en: "Monterrey Stadium · Monterrey"
    },
    es: {
      title: "Túnez vs Suecia",
      teams: "Túnez vs Suecia",
      location: "Monterrey Stadium · Monterrey",
      note: "Partido del Grupo F."
    },
    en: {
      title: "Tunisia vs Sweden",
      teams: "Tunisia vs Sweden",
      location: "Monterrey Stadium · Monterrey",
      note: "Group F match."
    }
  },
  {
    id: 33,
    date: "2026-06-20",
    officialTime: "13:00 ET",
    kickoffUtc: "2026-06-20T17:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "NED",
      flag: "NED",
      es: "Países Bajos",
      en: "Netherlands"
    },
    away: {
      code: "SWE",
      flag: "SWE",
      es: "Suecia",
      en: "Sweden"
    },
    stadium: {
      es: "Houston Stadium · Houston",
      en: "Houston Stadium · Houston"
    },
    es: {
      title: "Países Bajos vs Suecia",
      teams: "Países Bajos vs Suecia",
      location: "Houston Stadium · Houston",
      note: "Segundo partido de Países Bajos en el Grupo F."
    },
    en: {
      title: "Netherlands vs Sweden",
      teams: "Netherlands vs Sweden",
      location: "Houston Stadium · Houston",
      note: "Netherlands’ second match in Group F."
    }
  },
  {
    id: 34,
    date: "2026-06-21",
    officialTime: "00:00 ET",
    kickoffUtc: "2026-06-21T04:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "TUN",
      flag: "TUN",
      es: "Túnez",
      en: "Tunisia"
    },
    away: {
      code: "JPN",
      flag: "JPN",
      es: "Japón",
      en: "Japan"
    },
    stadium: {
      es: "Monterrey Stadium · Monterrey",
      en: "Monterrey Stadium · Monterrey"
    },
    es: {
      title: "Túnez vs Japón",
      teams: "Túnez vs Japón",
      location: "Monterrey Stadium · Monterrey",
      note: "Partido del Grupo F."
    },
    en: {
      title: "Tunisia vs Japan",
      teams: "Tunisia vs Japan",
      location: "Monterrey Stadium · Monterrey",
      note: "Group F match."
    }
  },
  {
    id: 35,
    date: "2026-06-25",
    officialTime: "19:00 ET",
    kickoffUtc: "2026-06-25T23:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "TUN",
      flag: "TUN",
      es: "Túnez",
      en: "Tunisia"
    },
    away: {
      code: "NED",
      flag: "NED",
      es: "Países Bajos",
      en: "Netherlands"
    },
    stadium: {
      es: "Kansas City Stadium · Kansas City",
      en: "Kansas City Stadium · Kansas City"
    },
    es: {
      title: "Túnez vs Países Bajos",
      teams: "Túnez vs Países Bajos",
      location: "Kansas City Stadium · Kansas City",
      note: "Partido final del Grupo F para Países Bajos."
    },
    en: {
      title: "Tunisia vs Netherlands",
      teams: "Tunisia vs Netherlands",
      location: "Kansas City Stadium · Kansas City",
      note: "Netherlands’ final Group F match."
    }
  },
  {
    id: 36,
    date: "2026-06-25",
    officialTime: "19:00 ET",
    kickoffUtc: "2026-06-25T23:00:00Z",
    stage: "Group Stage",
    group: "F",
    source: "FIFA",
    home: {
      code: "JPN",
      flag: "JPN",
      es: "Japón",
      en: "Japan"
    },
    away: {
      code: "SWE",
      flag: "SWE",
      es: "Suecia",
      en: "Sweden"
    },
    stadium: {
      es: "Dallas Stadium · Dallas",
      en: "Dallas Stadium · Dallas"
    },
    es: {
      title: "Japón vs Suecia",
      teams: "Japón vs Suecia",
      location: "Dallas Stadium · Dallas",
      note: "Partido final del Grupo F."
    },
    en: {
      title: "Japan vs Sweden",
      teams: "Japan vs Sweden",
      location: "Dallas Stadium · Dallas",
      note: "Final Group F match."
    }
  },
  {
    id: 37,
    date: "2026-06-15",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-15T19:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "BEL",
      flag: "BEL",
      es: "Bélgica",
      en: "Belgium"
    },
    away: {
      code: "EGY",
      flag: "EGY",
      es: "Egipto",
      en: "Egypt"
    },
    stadium: {
      es: "Seattle Stadium · Seattle",
      en: "Seattle Stadium · Seattle"
    },
    es: {
      title: "Bélgica vs Egipto",
      teams: "Bélgica vs Egipto",
      location: "Seattle Stadium · Seattle",
      note: "Primer partido del Grupo G."
    },
    en: {
      title: "Belgium vs Egypt",
      teams: "Belgium vs Egypt",
      location: "Seattle Stadium · Seattle",
      note: "Opening match of Group G."
    }
  },
  {
    id: 38,
    date: "2026-06-15",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-16T01:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "IRN",
      flag: "IRN",
      es: "Irán",
      en: "IR Iran"
    },
    away: {
      code: "NZL",
      flag: "NZL",
      es: "Nueva Zelanda",
      en: "New Zealand"
    },
    stadium: {
      es: "Los Angeles Stadium · Los Angeles",
      en: "Los Angeles Stadium · Los Angeles"
    },
    es: {
      title: "Irán vs Nueva Zelanda",
      teams: "Irán vs Nueva Zelanda",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Partido del Grupo G."
    },
    en: {
      title: "IR Iran vs New Zealand",
      teams: "IR Iran vs New Zealand",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Group G match."
    }
  },
  {
    id: 39,
    date: "2026-06-21",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-21T19:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "BEL",
      flag: "BEL",
      es: "Bélgica",
      en: "Belgium"
    },
    away: {
      code: "IRN",
      flag: "IRN",
      es: "Irán",
      en: "IR Iran"
    },
    stadium: {
      es: "Los Angeles Stadium · Los Angeles",
      en: "Los Angeles Stadium · Los Angeles"
    },
    es: {
      title: "Bélgica vs Irán",
      teams: "Bélgica vs Irán",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Segundo partido de Bélgica en el Grupo G."
    },
    en: {
      title: "Belgium vs IR Iran",
      teams: "Belgium vs IR Iran",
      location: "Los Angeles Stadium · Los Angeles",
      note: "Belgium’s second match in Group G."
    }
  },
  {
    id: 40,
    date: "2026-06-21",
    officialTime: "21:00 ET",
    kickoffUtc: "2026-06-22T01:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "NZL",
      flag: "NZL",
      es: "Nueva Zelanda",
      en: "New Zealand"
    },
    away: {
      code: "EGY",
      flag: "EGY",
      es: "Egipto",
      en: "Egypt"
    },
    stadium: {
      es: "BC Place Vancouver · Vancouver",
      en: "BC Place Vancouver · Vancouver"
    },
    es: {
      title: "Nueva Zelanda vs Egipto",
      teams: "Nueva Zelanda vs Egipto",
      location: "BC Place Vancouver · Vancouver",
      note: "Partido del Grupo G."
    },
    en: {
      title: "New Zealand vs Egypt",
      teams: "New Zealand vs Egypt",
      location: "BC Place Vancouver · Vancouver",
      note: "Group G match."
    }
  },
  {
    id: 41,
    date: "2026-06-26",
    officialTime: "23:00 ET",
    kickoffUtc: "2026-06-27T03:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "EGY",
      flag: "EGY",
      es: "Egipto",
      en: "Egypt"
    },
    away: {
      code: "IRN",
      flag: "IRN",
      es: "Irán",
      en: "IR Iran"
    },
    stadium: {
      es: "Seattle Stadium · Seattle",
      en: "Seattle Stadium · Seattle"
    },
    es: {
      title: "Egipto vs Irán",
      teams: "Egipto vs Irán",
      location: "Seattle Stadium · Seattle",
      note: "Partido final del Grupo G."
    },
    en: {
      title: "Egypt vs IR Iran",
      teams: "Egypt vs IR Iran",
      location: "Seattle Stadium · Seattle",
      note: "Final Group G match."
    }
  },
  {
    id: 42,
    date: "2026-06-26",
    officialTime: "23:00 ET",
    kickoffUtc: "2026-06-27T03:00:00Z",
    stage: "Group Stage",
    group: "G",
    source: "FIFA",
    home: {
      code: "NZL",
      flag: "NZL",
      es: "Nueva Zelanda",
      en: "New Zealand"
    },
    away: {
      code: "BEL",
      flag: "BEL",
      es: "Bélgica",
      en: "Belgium"
    },
    stadium: {
      es: "BC Place Vancouver · Vancouver",
      en: "BC Place Vancouver · Vancouver"
    },
    es: {
      title: "Nueva Zelanda vs Bélgica",
      teams: "Nueva Zelanda vs Bélgica",
      location: "BC Place Vancouver · Vancouver",
      note: "Partido final del Grupo G."
    },
    en: {
      title: "New Zealand vs Belgium",
      teams: "New Zealand vs Belgium",
      location: "BC Place Vancouver · Vancouver",
      note: "Final Group G match."
    }
  },
  {
    id: 43,
    date: "2026-06-15",
    officialTime: "12:00 ET",
    kickoffUtc: "2026-06-15T16:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "ESP",
      flag: "ESP",
      es: "España",
      en: "Spain"
    },
    away: {
      code: "CPV",
      flag: "CPV",
      es: "Cabo Verde",
      en: "Cape Verde"
    },
    stadium: {
      es: "Atlanta Stadium · Atlanta",
      en: "Atlanta Stadium · Atlanta"
    },
    es: {
      title: "España vs Cabo Verde",
      teams: "España vs Cabo Verde",
      location: "Atlanta Stadium · Atlanta",
      note: "Primer partido de España en el Grupo H."
    },
    en: {
      title: "Spain vs Cape Verde",
      teams: "Spain vs Cape Verde",
      location: "Atlanta Stadium · Atlanta",
      note: "Spain’s opening match in Group H."
    }
  },
  {
    id: 44,
    date: "2026-06-15",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-15T22:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "KSA",
      flag: "KSA",
      es: "Arabia Saudita",
      en: "Saudi Arabia"
    },
    away: {
      code: "URU",
      flag: "URU",
      es: "Uruguay",
      en: "Uruguay"
    },
    stadium: {
      es: "Miami Stadium · Miami",
      en: "Miami Stadium · Miami"
    },
    es: {
      title: "Arabia Saudita vs Uruguay",
      teams: "Arabia Saudita vs Uruguay",
      location: "Miami Stadium · Miami",
      note: "Partido del Grupo H."
    },
    en: {
      title: "Saudi Arabia vs Uruguay",
      teams: "Saudi Arabia vs Uruguay",
      location: "Miami Stadium · Miami",
      note: "Group H match."
    }
  },
  {
    id: 45,
    date: "2026-06-21",
    officialTime: "12:00 ET",
    kickoffUtc: "2026-06-21T16:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "ESP",
      flag: "ESP",
      es: "España",
      en: "Spain"
    },
    away: {
      code: "KSA",
      flag: "KSA",
      es: "Arabia Saudita",
      en: "Saudi Arabia"
    },
    stadium: {
      es: "Atlanta Stadium · Atlanta",
      en: "Atlanta Stadium · Atlanta"
    },
    es: {
      title: "España vs Arabia Saudita",
      teams: "España vs Arabia Saudita",
      location: "Atlanta Stadium · Atlanta",
      note: "Segundo partido de España en el Grupo H."
    },
    en: {
      title: "Spain vs Saudi Arabia",
      teams: "Spain vs Saudi Arabia",
      location: "Atlanta Stadium · Atlanta",
      note: "Spain’s second match in Group H."
    }
  },
  {
    id: 46,
    date: "2026-06-21",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-21T22:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "URU",
      flag: "URU",
      es: "Uruguay",
      en: "Uruguay"
    },
    away: {
      code: "CPV",
      flag: "CPV",
      es: "Cabo Verde",
      en: "Cape Verde"
    },
    stadium: {
      es: "Miami Stadium · Miami",
      en: "Miami Stadium · Miami"
    },
    es: {
      title: "Uruguay vs Cabo Verde",
      teams: "Uruguay vs Cabo Verde",
      location: "Miami Stadium · Miami",
      note: "Partido del Grupo H."
    },
    en: {
      title: "Uruguay vs Cape Verde",
      teams: "Uruguay vs Cape Verde",
      location: "Miami Stadium · Miami",
      note: "Group H match."
    }
  },
  {
    id: 47,
    date: "2026-06-26",
    officialTime: "20:00 ET",
    kickoffUtc: "2026-06-27T00:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "CPV",
      flag: "CPV",
      es: "Cabo Verde",
      en: "Cape Verde"
    },
    away: {
      code: "KSA",
      flag: "KSA",
      es: "Arabia Saudita",
      en: "Saudi Arabia"
    },
    stadium: {
      es: "Houston Stadium · Houston",
      en: "Houston Stadium · Houston"
    },
    es: {
      title: "Cabo Verde vs Arabia Saudita",
      teams: "Cabo Verde vs Arabia Saudita",
      location: "Houston Stadium · Houston",
      note: "Partido final del Grupo H."
    },
    en: {
      title: "Cape Verde vs Saudi Arabia",
      teams: "Cape Verde vs Saudi Arabia",
      location: "Houston Stadium · Houston",
      note: "Final Group H match."
    }
  },
  {
    id: 48,
    date: "2026-06-26",
    officialTime: "20:00 ET",
    kickoffUtc: "2026-06-27T00:00:00Z",
    stage: "Group Stage",
    group: "H",
    source: "FIFA",
    home: {
      code: "URU",
      flag: "URU",
      es: "Uruguay",
      en: "Uruguay"
    },
    away: {
      code: "ESP",
      flag: "ESP",
      es: "España",
      en: "Spain"
    },
    stadium: {
      es: "Guadalajara Stadium · Guadalajara",
      en: "Guadalajara Stadium · Guadalajara"
    },
    es: {
      title: "Uruguay vs España",
      teams: "Uruguay vs España",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Partido final del Grupo H."
    },
    en: {
      title: "Uruguay vs Spain",
      teams: "Uruguay vs Spain",
      location: "Guadalajara Stadium · Guadalajara",
      note: "Final Group H match."
    }
  },
  {
    id: 49,
    date: "2026-06-16",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-16T19:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "FRA",
      flag: "FRA",
      es: "Francia",
      en: "France"
    },
    away: {
      code: "SEN",
      flag: "SEN",
      es: "Senegal",
      en: "Senegal"
    },
    stadium: {
      es: "New York New Jersey Stadium · East Rutherford",
      en: "New York New Jersey Stadium · East Rutherford"
    },
    es: {
      title: "Francia vs Senegal",
      teams: "Francia vs Senegal",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Primer partido de Francia en el Grupo I."
    },
    en: {
      title: "France vs Senegal",
      teams: "France vs Senegal",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "France’s opening match in Group I."
    }
  },
  {
    id: 50,
    date: "2026-06-16",
    officialTime: "18:00 ET",
    kickoffUtc: "2026-06-16T22:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "IRQ",
      flag: "IRQ",
      es: "Irak",
      en: "Iraq"
    },
    away: {
      code: "NOR",
      flag: "NOR",
      es: "Noruega",
      en: "Norway"
    },
    stadium: {
      es: "Boston Stadium · Foxborough",
      en: "Boston Stadium · Foxborough"
    },
    es: {
      title: "Irak vs Noruega",
      teams: "Irak vs Noruega",
      location: "Boston Stadium · Foxborough",
      note: "Partido del Grupo I."
    },
    en: {
      title: "Iraq vs Norway",
      teams: "Iraq vs Norway",
      location: "Boston Stadium · Foxborough",
      note: "Group I match."
    }
  },
  {
    id: 51,
    date: "2026-06-22",
    officialTime: "17:00 ET",
    kickoffUtc: "2026-06-22T21:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "FRA",
      flag: "FRA",
      es: "Francia",
      en: "France"
    },
    away: {
      code: "IRQ",
      flag: "IRQ",
      es: "Irak",
      en: "Iraq"
    },
    stadium: {
      es: "Philadelphia Stadium · Philadelphia",
      en: "Philadelphia Stadium · Philadelphia"
    },
    es: {
      title: "Francia vs Irak",
      teams: "Francia vs Irak",
      location: "Philadelphia Stadium · Philadelphia",
      note: "Segundo partido de Francia en el Grupo I."
    },
    en: {
      title: "France vs Iraq",
      teams: "France vs Iraq",
      location: "Philadelphia Stadium · Philadelphia",
      note: "France’s second match in Group I."
    }
  },
  {
    id: 52,
    date: "2026-06-22",
    officialTime: "20:00 ET",
    kickoffUtc: "2026-06-23T00:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "NOR",
      flag: "NOR",
      es: "Noruega",
      en: "Norway"
    },
    away: {
      code: "SEN",
      flag: "SEN",
      es: "Senegal",
      en: "Senegal"
    },
    stadium: {
      es: "New York New Jersey Stadium · East Rutherford",
      en: "New York New Jersey Stadium · East Rutherford"
    },
    es: {
      title: "Noruega vs Senegal",
      teams: "Noruega vs Senegal",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Partido del Grupo I."
    },
    en: {
      title: "Norway vs Senegal",
      teams: "Norway vs Senegal",
      location: "New York New Jersey Stadium · East Rutherford",
      note: "Group I match."
    }
  },
  {
    id: 53,
    date: "2026-06-26",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-26T19:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "NOR",
      flag: "NOR",
      es: "Noruega",
      en: "Norway"
    },
    away: {
      code: "FRA",
      flag: "FRA",
      es: "Francia",
      en: "France"
    },
    stadium: {
      es: "Boston Stadium · Foxborough",
      en: "Boston Stadium · Foxborough"
    },
    es: {
      title: "Noruega vs Francia",
      teams: "Noruega vs Francia",
      location: "Boston Stadium · Foxborough",
      note: "Partido final del Grupo I para Francia."
    },
    en: {
      title: "Norway vs France",
      teams: "Norway vs France",
      location: "Boston Stadium · Foxborough",
      note: "France’s final Group I match."
    }
  },
  {
    id: 54,
    date: "2026-06-26",
    officialTime: "15:00 ET",
    kickoffUtc: "2026-06-26T19:00:00Z",
    stage: "Group Stage",
    group: "I",
    source: "FIFA",
    home: {
      code: "SEN",
      flag: "SEN",
      es: "Senegal",
      en: "Senegal"
    },
    away: {
      code: "IRQ",
      flag: "IRQ",
      es: "Irak",
      en: "Iraq"
    },
    stadium: {
      es: "Toronto Stadium · Toronto",
      en: "Toronto Stadium · Toronto"
    },
    es: {
      title: "Senegal vs Irak",
      teams: "Senegal vs Irak",
      location: "Toronto Stadium · Toronto",
      note: "Partido final del Grupo I."
    },
    en: {
      title: "Senegal vs Iraq",
      teams: "Senegal vs Iraq",
      location: "Toronto Stadium · Toronto",
      note: "Final Group I match."
    }
  }
];

console.log("✅ Renebook World Cup 2026 matches loaded:", window.RENEBOOK_WORLDCUP_2026_MATCHES);
